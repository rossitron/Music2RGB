...
static arm_cfft_radix4_instance_q15 cfft_inst;

void AudioAnalyzeFFT256::init(void)
{
	// TODO: replace this with static const version
	arm_cfft_radix4_init_q15(&cfft_inst, FFTSIZE, 0, 1);
	...
	//arm_cfft_radix4_q15(&cfft_inst, buffer);
	//n = ARM_DWT_CYCCNT - n;
	//__enable_irq();
	//cycles = n;
...
void AudioAnalyzeFFT2048::init(void)
{
	// TODO: replace this with static const version
	//arm_cfft_radix4_init_q15(&cfft_inst, FFTSIZE, 0, 1);
	arm_rfft_init_q15(&rfft_inst, &cfft_inst, FFTSIZE, 0, 1);
}
...
void AudioAnalyzeFFT1024::update(void)
{
	...
	copy_to_fft_buffer(buffer, prevblock->data);
	copy_to_fft_buffer(buffer+FFTSIZE, block->data);
	//window = AudioWindowBlackmanNuttall256;
	//window = NULL;
	if (window) apply_window_to_fft_buffer(buffer, window);
	arm_cfft_radix4_q15(&cfft_inst, buffer);
	...
	// supposed to average the magnitude squared, then do the square
	// root at the end after dividing by naverage.
	arm_cmplx_mag_q15(buffer, buffer, (FFTSIZE/2));
	if (count == 0) {
	...
	if (state == 0) {
		...
		//Serial.print("2");
		arm_cmplx_mag_q15(buffer, output, (FFTSIZE/2));
		outputflag = true;
		if (block == NULL) return;
		copy_to_fft_buffer(buffer, block->data);
		state = 1;
...
void AudioAnalyzeFFT2048::update(void)
{
	audio_block_t *block;
	...
	if (window) apply_window_to_fft_buffer(buffer, window);
	//arm_cfft_radix4_q15(&cfft_inst, buffer);
	arm_rfft_q15(&rfft_inst, buffer, buffer);
...
#define PDB_PERIOD 96e2 //1087 // 48e6 / 18662 // 96e6 / 38630
