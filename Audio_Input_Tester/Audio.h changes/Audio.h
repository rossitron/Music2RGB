...
extern const int16_t AudioWindowHanning2048[];
}
...
{
public:
	AudioAnalyzeFFT1024(uint8_t navg = 8, const int16_t *win = AudioWindowHanning1024)
	  : AudioStream(1, inputQueueArray), outputflag(false),
	    prevblock(NULL), count(0), naverage(navg), window(win)  { init(); }
	...
	virtual void update(void);
	//uint32_t cycles;
	int32_t output[512] __attribute__ ((aligned (4)));
private:
	void init(void);
	const int16_t *window;
	audio_block_t *prevblock;
...
class AudioAnalyzeFFT2048 : public AudioStream
{
public:
	AudioAnalyzeFFT2048(uint8_t navg = 8, const int16_t *win = AudioWindowHanning2048)
	  : AudioStream(1, inputQueueArray), outputflag(false),
	    prevblock(NULL), count(0), naverage(navg), window(win)  { init(); }
	...
	virtual void update(void);
	//uint32_t cycles;
	int32_t output[2048] __attribute__ ((aligned (4)));
private:
	void init(void);
	const int16_t *window;
	audio_block_t *prevblock;
	int16_t buffer[4096] __attribute__ ((aligned (4)));
	uint8_t count;
	uint8_t naverage;
	bool outputflag;
	audio_block_t *inputQueueArray[1];
